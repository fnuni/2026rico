...
**Affiliation:** University of Salento, Department of Engineering for Innovation  
...
├── README.md                           (this file)
├── data/
│   ├── tsp_instances_input.json        (20 synthetic TSP instances)
│   ├── tsp_results_output.json         (solver outputs for benchmarks)
...
### 1. **tsp_instances_input.json** (27.6 KB)
**Purpose:** Synthetic TSP instances for benchmarking  
**Content:** 20 instances with N ∈ {20, 30, 40, ..., 180} nodes  
**Format:** JSON array of objects  
**Fields:**
- `instance_id`: Unique identifier (e.g., "tsp_n050_05")
...
**Relation to Paper:** Corresponds to synthetic benchmark instances (Section 6.2, Table 3)

---

### 2. **tsp_results_output.json** (11.0 KB)
**Purpose:** Solver outputs demonstrating performance metrics  
**Content:** Results for 20 benchmark instances  
**Format:** JSON array of objects  
**Fields:**
- `instance_id`: Links to input instance
- `n_nodes`: Problem size
...
- `runtime_ms`: Execution time (milliseconds)
- `lk_runtime_ms`: LK baseline runtime
- `speedup`: Runtime ratio (LK / Hybrid)
- `tour`: Node visitation order (array of indices)
- `attention_precision_k100`: Precision@100 metric (Section 5.4)
...
Compare `tsp_results_output.json` statistics to Table 3 values  
...
Reviewers can request access to original data under NDA for verification purposes. Contact corresponding author: francesco.nucci@unisalento.it
...
**Questions about data:** Open an issue in this repository  
**Collaboration inquiries:** Contact via email  
**Original data access (under NDA):** Contact corresponding author
...
- Initial release with 3 synthetic datasets
- 20 benchmark instances + 847 warehouse missions
- Statistical properties validated against paper

**Planned (upon paper acceptance):**  
- Additional benchmark instances (N = 200-500)
- Pseudocode for attention-guided 2-opt
...
We thank our anonymous industrial partner for supporting this research and permitting publication of synthetic data replicating our deployment results. This work was supported by the University of Salento Research Base Funding.
